...
def main(args):
    ...
    args.save_dir = utils.get_save_dir( args.save_dir, args.rand_seed ,args.position, args.noise_type, args.subject_independent )
    ...
    if args.dataset == 'ISRUC':
        fold_clean, fold_contaminated, fold_len = get_isruc(args.data_dir, args.position, args.noise_type)
    if args.dataset == 'EEGDenoiseNet':
       train_loader, test_loader = get_EEGDenoiseNet()

    in_channels = fold_clean[0].shape[1]
    if args.subject_independent:
        DataGenerator = kFoldGenerator(fold_clean, fold_contaminated, args.batch_size , args.noise_type)
        histories = []
        for i in range(10):
            # Train
        ...
        histories = []
        train_loader, val_loader = prepare_data(fold_clean, fold_contaminated, args.batch_size, args.position)
        print(128*'-')
        model = make_model( args=args, in_channels=in_channels ,DEVICE=device )
        total_param = 0
        for param_tensor in model.state_dict():
